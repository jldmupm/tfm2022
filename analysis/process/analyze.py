...
def get_unique_from_mongo(field, collection=cfg.get_config().datasources.sensors.collection) -> List[str]:
    print('get_unique_from_mongo', field)
    lst = mg.get_mongodb_collection().distinct(field)
    print('get_unique_from_mongo', lst)
...
#def calculate_merged_data(start_at, end_at, category: str = cfg.get_config().data.feedback['category'], group_type: mg.GROUP_SENSORS_USING_TYPE = 'group_kind_sensor', callback_start_merging: Callable = None, callback_end_merging: Optional[Callable] = None) -> dd.DataFrame:
